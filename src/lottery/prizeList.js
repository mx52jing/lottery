...
import { currentPrizeIndex } from "./index";
      ...
      item.title
                            ...
                            <div class="prize-count">
                                <div class="progress">
                                    <div id="prize-bar-${
                                      item.type
                                    }" class="progress-bar progress-bar-danger progress-bar-striped active" style="width: 0;">
                                    </div>
                                </div>
                                <div id="prize-count-${
                                  item.type
  ...
  return function (currentPrizeIndex, count, isInit) {
    ...
    if (isInit) {
      for (let i = prizes.length - 1; i > currentPrizeIndex; i--) {
        const type = prizes[i]["type"],
          totalCount =  prizes[i]["count"];
        // 已经抽取了几个了
        let alreadyDrawnNum = 0;
        if(!!mockData.luckyData[i]) {
          alreadyDrawnNum = totalCount;
        }
        document.querySelector(`#prize-item-${type}`).className =
          "prize-item done";
        document.querySelector(`#prize-bar-${type}`).style.width = `${alreadyDrawnNum / totalCount * 100}%`;
    ...
    remainingCount = remainingCount < 0 ? totalCount : count;
    let percent = (remainingCount / totalCount).toFixed(2);
    elements.bar && (elements.bar.style.width = percent * 100 + "%");
    elements.text && (elements.text.textContent = remainingCount + "/" + totalCount);
...
// 切换左侧奖品列表盒子转动光圈
export const switchPrizeApertures = () => {
  let currentPrize = prizes[currentPrizeIndex],
    type = currentPrize.type,
    elements = prizeElement[type];
  if (!elements) {
