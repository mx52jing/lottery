...
import "./canvas.js";
import {addQipao, resetPrize, setPrizeData, setPrizes, showPrizeList, switchPrizeApertures } from "./prizeList";
import {NUMBER_MATRIX} from "./config.js";
import mockData from "./mock"
import {saveAndDownloadData, handleReLotteryBtnShow, setLotteryBtnText, isLotteryFinished } from "./util";
import enterMusic from '../data/lottery.mp3'
...
let TOTAL_CARDS,
  ...
  btns = {
    ...
    lotteryBtn: document.querySelector('#lottery'),
    reLotteryBtn: document.querySelector('#reLottery')
  },
  prizes,
  EACH_COUNT,
  ROW_COUNT = 7,
...
function bindEvent() {
  document.querySelector("#menu").addEventListener("click", async function (e) {
    e.stopPropagation();
    // 如果正在抽奖，则禁止一切操作'
    let target = e.target.id;
    if(target === 'lottery') {
    ...
    switch (target) {
      ...
      case "lottery":
        ...
        btns.lotteryBtn.innerText = '停止抽奖'
        // 切换音乐
      ...
      case "reLottery":
        ...
        if(!isLotting && +currentPrizeIndex <= 0) {
          alert('抽奖已经结束啦~~')
          addQipao(`抽奖已经结束啦~~`);
          return;
        }
...
function rotateBall() {
  return new Promise((resolve, reject) => {
    scene.rotation.y = 0;
    // 这里可以换成手动控制抽奖停止, 这里修改球将会无限旋转，除非调用stop函数
    rotateObj = new TWEEN.Tween(scene.rotation);
    rotateObj
      .to(
        {
          y: Math.PI * (currentPrize && currentPrize.circle || 8) * ROTATE_LOOP
        },
        (currentPrize && currentPrize.ROTATE_TIME || ROTATE_TIME) * ROTATE_LOOP
      )
      .onUpdate(render)
      .start()
      .onStop(() => {
 ...
 * 抽奖
 */
async function lottery(customCount) {
  await rotateBall();
  // 将之前的记录置空
  currentLuckys = [];
  selectedCardIndex = [];
  ...
  let perCount = EACH_COUNT[currentPrizeIndex],
    ...
    luckyDataLength = (luckyData ? luckyData.length : 0),
    leftCount = basicData.leftUsers.length,
    leftPrizeCount = currentPrize.count - luckyDataLength,
    // 剩余可抽取的奖品数
    remainingCount = perCount > leftPrizeCount ? leftPrizeCount : perCount
  ...
  currentLuckys = lotteryRan(leftCount, remainingCount).map(index => {
    return cloneLeftUsers[index]
  })
  console.log('中将人员id', currentLuckys);
...
window.onload = function () {
  ...
  setTimeout(function () {
    replaceMusic(enterMusic)
  }, 2000);
};

export { isLotting, currentPrizeIndex, btns, currentPrize, basicData, currentLuckys };
